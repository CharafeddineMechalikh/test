...
public class DefaultComputingNode extends LocationAwareNode {
	...
	private SubTask getSubTaskById(SubTask task, String id) {
		for(int i=0; i< task.getParentTask().getSubTasks().size();i++) {
			if (task.getParentTask().getSubTasks().get(i).getId()==Integer.parseInt(id))
				return task.getParentTask().getSubTasks().get(i);
		}
		return null;
	}
	...
	public void submitTask(Task task) {
		...
		boolean wait = false;
		for(int j=0; j<((SubTask)task).getRequirements().length;j++) { //browse there requirements
			
			if(((SubTask)task).getRequirements()[j].equals("")) continue;
			
			if(!getSubTaskById((SubTask)task, ((SubTask)task).getRequirements()[j]).hasBeenReturned()) {
			  wait = true;
			 System.out.println(this.getClass().getSimpleName()+ ", SubTask: "+ ((SubTask) task).getParentTask().getId()+"-"+task.getId()+"  is waiting for task:"+getSubTaskById((SubTask)task, ((SubTask)task).getRequirements()[j]).getId());
			
			}
			else {
			 System.err.println(this.getClass().getSimpleName()+ ", SubTask: "+ ((SubTask) task).getParentTask().getId()+"-"+task.getId()+"  is no more waiting for task:"+getSubTaskById((SubTask)task, ((SubTask)task).getRequirements()[j]).getId());	
			}
		}
		
		if(wait) {
			((SubTask)task).setWaiting(true);
			return;
		}
